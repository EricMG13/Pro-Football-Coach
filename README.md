...
| `Sources/CoachWorldApp/` | The composition layer — the application root, the store, the save coordinator, and one read-model provider per screen family. The only module allowed to see both `GameState` and the screen read models |
...
calibration or archive run cannot contaminate another: `accessibility`, `app` (XcodeGen + a generic iOS
build), `archive`, `calibration`, `core`, `determinism`, `full`, `release` and `soaks`.
...
import`s all three library modules, and SwiftPM has no reason to infer testability for one — so a release
