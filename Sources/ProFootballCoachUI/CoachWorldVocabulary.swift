...
struct CoachWorldRatingRing: View {
    ...
    /// The design's own banding, diverging around a neutral centre: an average figure is neither
    /// praised nor flagged, and colour is spent only at the ends. Red for a real weakness, amber
    /// below par, green for genuinely above — lightened at 80-84 and full at 85 up, so "good" and
    /// "excellent" are not the same green. Colour always repeats the printed number beside it.
    /// `CoachWorldTokens.Heat.color` is the one definition of the banding, so this stays a delegate
