...
public struct RosterReadModel: Sendable, Equatable {
    public struct PlayerRow: Identifiable, Sendable, Equatable {
        ...
        public init(stableID: String, person: CoachWorldPersonReference, number: Int, position: String, academicYear: String, rosterRole: String, overall: Int, development: Int, schemeFit: String, condition: Int, availability: String, profile: PlayerProfileReadModel) {
            self.stableID = stableID
            self.person = person
            self.number = number
            self.position = position
    ...
    public init(snapshotID: String, provenance: CoachWorldDataProvenance, world: CoachWorldReference, team: CoachWorldTeamReference, coach: CoachWorldPersonReference, seasonLabel: String, weekLabel: String, recordLabel: String, rankLabel: String?, rosterLimit: Int, injuryCount: Int, openNeedCount: Int, players: [PlayerRow]) {
    ...
    public func sorted(_ players: [RosterReadModel.PlayerRow]) -> [RosterReadModel.PlayerRow] {
        players.sorted { lhs, rhs in
            ...
            case .development:
                ...
                return isAscending ? lhs.development < rhs.development : lhs.development > rhs.development
            case .condition:
...
public extension CoachWorldSampleData {
    static let roster: RosterReadModel = {
        func makePlayer(id: String, number: Int, name: String, position: String, year: String, role: String, overall: Int, development: Int, fit: String, condition: Int, availability: String, hometown: String, strengths: [String], concern: String, summary: String, athletic: [Int], technical: [Int], mental: [Int], form: [Int]) -> RosterReadModel.PlayerRow {
            ...
            return .init(stableID: id, person: person, number: number, position: position, academicYear: year, rosterRole: role, overall: overall, development: development, schemeFit: fit, condition: condition, availability: availability, profile: profile)
        }
        ...
        return RosterReadModel(snapshotID: "sample-roster-snapshot", provenance: .sample, world: world, team: homeTeam, coach: headCoach, seasonLabel: "2027 season", weekLabel: "Week 9", recordLabel: "6–2", rankLabel: "#19", rosterLimit: 85, injuryCount: 2, openNeedCount: 3, players: [
            makePlayer(id: "sample-roster-bishop", number: 12, name: "Andre Bishop", position: "QB", year: "SR", role: "Captain · Starter", overall: 91, development: 78, fit: "Elite", condition: 96, availability: "Available", hometown: "Calder Springs, Thornby Reach", strengths: ["Deep accuracy", "Pressure control"], concern: "Late movement can hold the ball too long", summary: "Commands the offense and protects high-leverage downs.", athletic: [82, 76, 88], technical: [92, 94, 89], mental: [91, 93, 90], form: [88, 91, 86]),
            makePlayer(id: "sample-roster-ward", number: 24, name: "Jalen Ward", position: "RB", year: "JR", role: "Starter", overall: 88, development: 86, fit: "Strong", condition: 91, availability: "Available", hometown: "Marrow Bend, Fenmark Flats", strengths: ["Contact balance", "Cut timing"], concern: "Pass protection remains inconsistent", summary: "Creates efficient early downs without wasting carries.", athletic: [91, 87, 90], technical: [86, 90, 84], mental: [85, 87, 79], form: [90, 84, 89]),
            makePlayer(id: "sample-roster-okafor", number: 6, name: "Miles Okafor", position: "WR", year: "SO", role: "Rotation", overall: 84, development: 92, fit: "Strong", condition: 100, availability: "Available", hometown: "Harrow Landing, Redmoor Coast", strengths: ["Release burst", "Open-field acceleration"], concern: "Boundary route detail is unfinished", summary: "The highest-upside receiver on the roster.", athletic: [94, 72, 86], technical: [83, 85, 80], mental: [79, 82, 68], form: [82, 87, 80]),
            makePlayer(id: "sample-roster-alvarez", number: 72, name: "Tomas Alvarez", position: "OT", year: "SR", role: "Starter", overall: 87, development: 74, fit: "Elite", condition: 88, availability: "Limited", hometown: "Pellham Mills, Dunmore Basin", strengths: ["Pass anchor", "Length"], concern: "Ankle limits lateral recovery", summary: "Reliable blind-side protection when healthy.", athletic: [76, 93, 82], technical: [90, 91, 88], mental: [86, 89, 84], form: [85, 88, 83]),
            makePlayer(id: "sample-roster-webb", number: 87, name: "Darius Webb", position: "EDGE", year: "JR", role: "Starter", overall: 89, development: 88, fit: "Elite", condition: 94, availability: "Available", hometown: "Larkin Crossing, Gallow Uplands", strengths: ["First step", "Counter timing"], concern: "Can lose run leverage chasing pressure", summary: "Changes passing downs and forces protection help.", athletic: [93, 86, 89], technical: [88, 92, 81], mental: [84, 86, 78], form: [92, 89, 90]),
            makePlayer(id: "sample-roster-reed", number: 55, name: "Marcus Reed", position: "MLB", year: "SR", role: "Captain · Starter", overall: 90, development: 76, fit: "Strong", condition: 97, availability: "Available", hometown: "Oakhaven Bluff, Netherby Divide", strengths: ["Run fits", "Communication"], concern: "Man coverage range is ordinary", summary: "Sets the front and prevents alignment errors.", athletic: [84, 89, 91], technical: [87, 90, 82], mental: [94, 95, 93], form: [89, 90, 91]),
            makePlayer(id: "sample-roster-brooks", number: 9, name: "Elijah Brooks", position: "CB", year: "SO", role: "Nickel", overall: 82, development: 94, fit: "Good", condition: 99, availability: "Available", hometown: "Wexford Harbor, Yarrow Tidelands", strengths: ["Press timing", "Recovery speed"], concern: "Route recognition varies snap to snap", summary: "Already playable inside with outside-corner upside.", athletic: [95, 70, 87], technical: [82, 84, 79], mental: [76, 78, 71], form: [81, 85, 83]),
        ])
    }()
}
#endif
