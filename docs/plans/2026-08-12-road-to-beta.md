...
| G-16 | **`Player` carries no jersey number.** Discovered 2026-08-13 while writing G-01's providers. `RosterReadModel.PlayerRow.number` and `PlayerProfileReadModel.number` are non-optional `Int`s, so a truthful Roster or Player Profile cannot be built without one, and deriving a number in the read model would invent a football fact the root does not hold. It belongs in generation, deterministically, legal by position band — which makes it a `GameState` schema change and a re-pin of both architecture fingerprints | Not started | Roster and Player Profile providers; box score; match-day actors |
