...
- [ ] Cross-process determinism proven: same seed, two separate process invocations, identical
      play-by-play hash.
- [ ] **Every pinned fingerprint is a measured value, and the commit that changed it says so.**
      Added 2026-08-13. The pins exist to make an engine change visible, so a pin edited to match
      whatever the code now produces is worse than a red one — it passes while proving nothing. Two
      questions, both answerable from the diff: was each pin re-measured by running the binary twice
      in *separate processes*, and does something in the same change explain why it moved? A moved
      `pinnedRootFingerprint` in particular means generated bytes changed, which is a short list:
      generation, the player or organisation model, or a trait activation.
