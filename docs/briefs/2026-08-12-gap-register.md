...
| Today | **Status update 2026-08-13, this row is stale.** `OrientationPolicyTest` now exists (`Tests/SimTests/Suites/DesignContractTests.swift`, "Orientation policy" suite) and reads `App/project.yml`, asserting both sensor orientations and no portrait. `04` §7's window was rewritten under D15 (852×393 through 956×440, 844×390 install floor). The two-tier `SmallestDeviceLayoutTest` (no-clipping at the install floor; full budget at the promise floor) is still genuinely missing, and — per this session's reading, with no simulator available to confirm — likely cannot be a headless `SimTests` assertion at all, since it needs actual rendered geometry; it may belong with the other owner-verifiable device work in `docs/PRE-DEPLOYMENT-CHECKLIST.md` §4 rather than the machine-gated half. |
| Delta | Owner decides D15 — **done, 2026-08-12.** `04` §7 window rewritten from verified sizes (sourcing rows Q4–Q5) — **done.** `OrientationPolicyTest` actually written — **done.** Two-tier `SmallestDeviceLayoutTest` — **still open**, and its right home (headless text check vs. simulator-rendered owner verification) is itself an open question this session could not resolve without a simulator to test against. |
