...
2. **The pre-snap field draws twenty-two dots in one vertical column**, because `playback` is nil before the first snap and the pre-existing static path stacks all eleven a side at one x with y by array index. The first thing a player sees is a column that snaps into a real formation after the first play. **Not fixed.**
3. **Dots are 20 pt and overlap heavily** across the interior line at this field scale. **Not fixed.**
