...
| HQ / staff recommendation | Recommend a weekly choice with confidence | Mock staff recommendation; optional `staffRecommendation` in `ScreenReadModels` | Omit | Conditional recommendation code is dormant because production supplies nil; no recommendation surface is rendered in this slice | Omission avoids attributing unsupported advice to staff |
...
| HQ / inbound correspondence feed | Read and answer desk correspondence | Mock correspondence feed; optional `correspondence` and `onOpenCorrespondence` | Omit | Conditional correspondence code is dormant because production supplies an empty collection; no correspondence surface is rendered in this slice | No unavailable inbox content is exposed |
...
| Profile / back route | Return to personnel context | Mock back control; `onClose` | Adapt | Preserve `Back to the roster` and the existing callback | Back action is first in the logical order and named plainly |
