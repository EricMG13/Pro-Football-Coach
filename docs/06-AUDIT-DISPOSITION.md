...
| 17 | Landscape declared supported, but the arcade's fixed-height layout puts play-call controls off-screen | **(i)** Portrait-only and iPhone-only are declared in `App/project.yml` (P1), and there is no arcade. **Test:** `OrientationPolicyTest` reads the project manifest. |
