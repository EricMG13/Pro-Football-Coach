...
| Contrast floors for team colours | 4.5:1 for `team.onTeam` on `team.primary`; 3:1 for `team.secondary` on `team.primary` | `04` §2.1's table. Both checked *at generation time*, so a **generated** pair that cannot carry legible text is regenerated rather than shipped. Requiring one foreground to work on *both* members was tried first and is unsatisfiable — it rules out every dark-plus-light identity. **The shipped world is not generated colour.** `CanonicalTeamBranding` overrides every pair at `worldSeed` without passing through `ColourGenerator`, so the retry loop never sees it: 36 of its 166 pairs fail this floor and 148 collide on trade dress, both owner-approved on 2026-08-23 and pinned by team id in `LegalTests.swift` |
...
| Leagues the legal tests sweep | 200 synthetic, **plus the canonical world** — 201 in all | The 200 match D6's falsifier sample, so one generation run serves both. The canonical world was added 2026-08-23 (`ca1401ca`): `CanonicalTeamBranding` returns early at every seed but its own, so a sweep of arbitrary seeds reads generator output the shipped world replaces, and the one league every tester sees was the one league no legal test read |
