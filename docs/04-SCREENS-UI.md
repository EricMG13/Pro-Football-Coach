...
Header: "2026 Season", team chip, Reputation. Team card: Overall (e.g. 76.5), record, collapsible schemes row, orange **Auto-Sort**, red **Injury Report**. Sections Offense (count chip) / Defense / Special Teams; per-position cards ("QB — 3 players"): ranked rows = number circle, name, OVR (tier-coloured, tier also spoken by VoiceOver: elite 90+ / star 84–89 / starter 74–83 / rotational 64–73 / fringe <64 — bands and AA-verified hexes live in `RatingTier`), age chip + years-pro chip (replaces college class/RS), status pill STARTER (green tint) / BACKUP (yellow) / reserve, injury icon. Drag to reorder within position. Roster count "53/53" + practice squad section "PS — 16" (elevate/send-down actions). Cut player action (→ dead-money confirm dialog, Front Office rules).
