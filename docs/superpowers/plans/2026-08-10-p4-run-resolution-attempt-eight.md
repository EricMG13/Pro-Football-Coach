...
Expected: non-zero exit; the new run-shape test passes and the only failures are the two pinned cross-process fingerprint assertions, which have not yet been updated.
