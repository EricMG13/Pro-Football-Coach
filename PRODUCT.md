...
- The simulation model is never simplified for presentation. The fast loop compresses how consequences are *shown*, never how they are computed (R2 T2).
- **One engine, one truth**, stated precisely: every mode runs the same `GameSimulator`, and every **engine-resolved** game — simmed or watched — produces statistically identical results, asserted by a mode-parity test. A game the player physically plays is the one exception, and it is deliberate: user skill is an input the calibration bands do not model, so user-played games are excluded from calibration and a sharp thumb may beat the sim with the same roster. The bounded skill ceiling, unchanged owner expectations, and difficulty setting are the counterweights (`06-PLAYED-GAME-MODE.md` §5). What is *never* permitted is the same engine producing different distributions for simmed versus watched games — that divergence is the scandal this project's research names by name.
